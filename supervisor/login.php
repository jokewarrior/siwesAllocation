...
if (isset($_POST['login_submit'])) {
    ...
    //confirm existence
    $check = "SELECT * FROM supervisor WHERE Password = '$password_hash' && Email = '$email'  LIMIT 1";
    $result = mysqli_query($link, $check);
    if (mysqli_num_rows($result)) {
        $row = mysqli_fetch_assoc($result);
        $edit = (empty($row['Name'])) ? true : false;
        $_SESSION['sup_id'] = $row['id'];
        ($edit) ? header("location:./first_log.php") : header("location:./home");
    } else {
...
<html lang="en">
    ...
    <body style="overflow-y:hidden; background: rgba(31,82,93,0.9);color:#fff;">
                    ...
                    <form class="loginForm" action="" method="POST">
                        <div class="form-group col-sm-4 col-sm-offset-4 col-xs-8 col-xs-offset-2">
                            <div class="input-group">
                                ...
                                <input style="height:40px" type="email" maxlength="50" name="email" placeholder="Email" class="form-control" required="" />
                            </div>
                            ...
                            <div class="input-group center-block" style="margin-top: 20px;">
                                <button style="height:50px" type="submit" name="login_submit" class="btn btn-primary btn-lg">
